...
const groupedByTeam = groupBy(data, "group");
//split object into team arrays
const teamList = Object.entries(groupedByTeam);

function showTeams() {
	teamList.forEach((team) => {
		//create divs for teams and show their names on DOM
		const teams = document.querySelector(".teams");
		const teamcard = document.createElement("div");
		teamcard.innerHTML = `<h1>Team ${team[0]}</h1>`;
		teams.appendChild(teamcard);
		//show asisstant and students name on cards
		const students = document.createElement("p");
		students.innerHTML = `<h2>${team[1][0].name}</h2> ${team[1]
			.slice(1) //first element is the assistant
			.map((student) => `</br>${student.name}`)}`;
		teamcard.appendChild(students);
		//add class list to team cards
		teamcard.classList.add("teams__card");
		//change background color as the group name
		teamcard.style.backgroundColor = team[0];
	});
}
showTeams(teamList);
